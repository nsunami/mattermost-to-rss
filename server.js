...
class MattermostClient {
  ...
  async getNewsChannelId() {
    ...
    try {
      ...
      const response = await this.client.get(`/teams/${this.config.teamId}/channels/name/${this.config.newsChannelName}`);
      return response.data.id;
    } catch (error) {
      console.error('Error fetching news channel:', error.response?.data || error.message);
      throw new Error(`Failed to find news channel: ${this.config.newsChannelName}`);
    }
  }
  ...
  async getNewsPosts(limit = 50) {
    try {
      ...
      console.error('Error fetching news posts:', error.response?.data || error.message);
  ...
  async getBotInfo() {
    try {
      ...
      return { username: 'api-user', first_name: 'Mattermost', last_name: 'API' };
    }
  }
}
  ...
  async generateFeed() {
    try {
      ...
      const feed = new RSS({
        ...
        description: channelInfo.purpose || `Latest posts from ${channelInfo.display_name}`,
        ...
        managingEditor: `${apiUserInfo.email || 'noreply@mattermost.com'} (${apiUserInfo.first_name} ${apiUserInfo.last_name})`,
        webMaster: `${apiUserInfo.email || 'noreply@mattermost.com'} (${apiUserInfo.first_name} ${apiUserInfo.last_name})`,
      ...
      newsPosts.forEach(post => {
        ...
        feed.item({
          ...
          description: this.formatDescription(post.message, post.props, post.author, post),
          url: postUrl,
          guid: post.id,
  ...
  formatDescription(message, props, author, post) {
    ...
    if (props && Object.keys(props).length > 0) {
      ...
      if (Object.keys(relevantProps).length > 0) {
        description += '<hr><small>Additional data: ' + JSON.stringify(relevantProps) + '</small>';
      }
    }
...
app.get('/posts', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to fetch news posts', message: error.message });
  }
});
...
app.listen(PORT, () => {
  ...
  console.log(`📰 Monitoring "${MATTERMOST_CONFIG.newsChannelName}" channel for news posts...`);
  ...
  if (!MATTERMOST_CONFIG.newsChannelId && !MATTERMOST_CONFIG.newsChannelName) {
    console.warn('⚠️  No news channel specified. Using default "news" channel name.');
  }
});
